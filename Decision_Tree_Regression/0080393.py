...
def decision_tree_regression_train(X_train, y_train, P):
    ...
    ##Looked from the lab for some parts
        ...
        for split_node in split_nodes:
            ...
            if len(data_indices) <= P:
                ...
                for split_position in split_positions:
                    ...
                    left_indices = data_indices[X_train[data_indices, 0] <= split_position]
                    right_indices = data_indices[X_train[data_indices, 0] > split_position]
                    
                    split_scores[split_position] = np.sum((y_train[left_indices] - np.mean(y_train[left_indices]))**2) + np.sum((y_train[right_indices] - np.mean(y_train[right_indices]))**2)
                ...
                if len(split_scores) > 0:
                    ...
                    node_indices[2 * split_node] = right_indices
    ...
    for terminal_node in terminal_nodes:
        ...
        while index > 1:
            ...
            if index % 2 != 0:
                ...
                rules.append("'x{} <= {:.2f}'".format(node_features[parent] + 1, node_splits[parent]))
